	...
	iss_date = iss_db_date[ 'date' ] 
        ...
        if iss_date.month == now.month:
          if iss_date.day == now.day:
            ...
            # print "date tstmp: %d " % timestamp_date
            iss_duration = getIssDurationFromDB( iss_db_date[ 'id' ] )
            timestamp_now = calendar.timegm( now.timetuple() )
	    # print "now: %d " % timestamp_now
            time_elapsed = ( timestamp_now - timestamp_date ) # (until positive : invisible) 
            time_left = ( timestamp_date + iss_duration[ 'duration' ] ) - timestamp_now # (as long as is positive : visible)
